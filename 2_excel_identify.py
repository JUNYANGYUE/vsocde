    ...
    for key, evaluations in groups.items():
        ...
        if evaluations:
            ...
            for evaluation in evaluations:
                ...
                content += f"    \\item \\textcolor{{red}}{{{user_text}：}}{evaluation}\n"
            content += "  \\end{itemize}\n"
...
# GUI setup
root = tk.Tk()
root.title("Excel处理程序 By音夬")
root.geometry('400x250')

title_level = StringVar(value="\\section")

section_radio = Radiobutton(root, text="Section", variable=title_level, value="\\section",height=2, width=50, font=('Times New Roman', 12))
section_radio.pack()

subsection_radio = Radiobutton(root, text="Subsection", variable=title_level, value="\\subsection",height=2, width=50, font=('Times New Roman', 12))
subsection_radio.pack()

user_text_label = tk.Label(root, text="输入文字:", height=2, width=50, font=('微软雅黑', 12))
user_text_label.pack()

user_text_entry = Entry(root, width=25)
user_text_entry.pack()

process_button = tk.Button(root, text="选择Excel文件并处理",width=18, height=2,font=('微软雅黑', 12),command=lambda: select_excel_file(title_level.get()))
